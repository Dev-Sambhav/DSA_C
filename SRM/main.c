...
void delete_record()
{
    ...
    while (fread(&s, sizeof(s), 1, fp) == 1)
    {
        /// check given record is exists or not
        if(strcmp(sname,s.name)==0){
            found = 1;
        }
        // storing all the student which is not matched in temp file.
        if (strcmp(sname, s.name) != 0)
        {
    ...
    if (found == 0)
    {
        ...
        printf("\n\t\tGiven Record Deleted Successfully");
    }
    // close both file
    fclose(fp);
    fclose(temp);
